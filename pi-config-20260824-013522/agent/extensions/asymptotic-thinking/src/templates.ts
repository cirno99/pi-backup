...
// ── 加载提示词模块（静态注册表，编译版替代动态 require）──
// 原实现用 jiti 动态 require("./prompts/...")，编译版无法打包动态路径，
// 改用 prompt-registry.ts 的静态路由表，查找顺序完全一致（精确→同大类general→全通用兜底）。
import { loadPromptModule } from "./prompt-registry";
