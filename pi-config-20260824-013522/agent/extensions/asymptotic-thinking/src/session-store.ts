// session-store.ts — 纯持久化层（SQLite，Node/bun 双运行时兼容）
// 只负责 load/save/delete，不包含任何状态机逻辑
//
// 兼容策略：优先 node:sqlite（Node 运行时），不可用时回退 bun:sqlite
// （pi 0.84.2 bun 编译二进制内置 bun:sqlite 但排除 node:sqlite）。
// 两个 API 均为 prepare().run/get/all + exec()，形状一致。
...
/** SQLite 驱动选择：node:sqlite → bun:sqlite 回退 */
function loadSqlite(): any {
  const req = createRequire(import.meta.url);
  try {
    // Node 运行时：node:sqlite 的 DatabaseSync
    const mod = req("node:sqlite");
    return { DatabaseSync: mod.DatabaseSync };
  } catch {
    try {
      // bun 运行时（含 pi bun 编译二进制）：bun:sqlite 的 Database
      const mod = req("bun:sqlite");
      return { DatabaseSync: mod.Database };
    } catch {
      // 双失败：抛错提示（不应发生）
      throw new Error(
        "[asymptotic-thinking] 无可用 SQLite 驱动：node:sqlite 与 bun:sqlite 均不可用"
      );
    }
  }
}
