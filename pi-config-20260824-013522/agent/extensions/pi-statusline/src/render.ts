...
export function renderAboveEditorWidget(width: number, theme: Theme, stats: Stats): string[] {
  ...
  // 目录/分支与模型信息合并为一行
  const combined = [line1, model].filter(Boolean).join("  ·  ");
  return [` ${truncateToWidth(combined, width - 1)}`];
}

export function calculateUsage(ctx: ExtensionContext): UsageStats {
  // 统计口径（避免“翻倍累加”）：
  // - input 取最后一次请求的真实输入（不含缓存）：每次调用的 usage.input 都包含完整历史
  //   上下文，逐条累加会随轮次翻倍虚高；最后一次输入即当前上下文/累计净输入。
  // - output 累加：每次调用只产生增量输出，累加即会话总生成量。
  // - cacheRead/cacheWrite 取最后一次（与 input 同口径，用于缓存命中数与命中率）。
  // - cost 累加：会话总成本。
  let input = 0;
  let output = 0;
  let cacheRead = 0;
  ...
  for (const e of ctx.sessionManager.getBranch()) {
    if (e.type === "message" && e.message.role === "assistant") {
      ...
      const usage = m.usage;
      // 防御：error/aborted 消息或旧版会话可能缺少 usage
      // 防御：error/aborted 消息或旧版会话可能缺少 usage
      if (!usage) continue;
      input = usage.input ?? 0;
...
export function buildFooterLeft(theme: Theme, stats: Stats): string {
  ...
  // Token 统计：输入 ↑ / 输出 ↓ / 缓存命中 R（左下角，同最后一次请求口径）
  const usage = stats.cachedUsage;
  if (usage.input > 0 || usage.output > 0) {
    parts.push(
      theme.fg("syntaxVariable", `↑${formatTokens(usage.input)}`),
      theme.fg("syntaxString", `↓${formatTokens(usage.output)}`),
    );
    // 缓存命中 token 数（R = cacheRead），跟在输入输出后面
    if (usage.cacheRead > 0) {
      parts.push(theme.fg("syntaxComment", `R${formatTokens(usage.cacheRead)}`));
    }
  }

  // 缓存命中率 = cacheRead / (cacheRead + cacheWrite + input)
  //   cache = syntaxComment
  if (usage.cacheRead > 0 || usage.input > 0) {
    const totalInput = usage.cacheRead + usage.cacheWrite + usage.input;
    const hitRate = Math.min((usage.cacheRead / totalInput) * 100, 100);
    parts.push(theme.fg("syntaxComment", `${hitRate.toFixed(2)}%c`));
  }
