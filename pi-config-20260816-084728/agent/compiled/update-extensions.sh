...
#   ./update-extensions.sh --skip-npm     更新 settings 包 + 打包，跳过 npm update
#
# 完整流程：
#   1) pi update --extensions        更新 settings.json packages 里的包
#   2) npm update（npm/ 目录）        更新所有 dependencies（含 pi-readseek 这类
...
#     pi update 不认识它（会报 No matching package），所以靠步骤 2 的 npm update 更新。
#   - 换电脑/迁移后：装好 pi、拷 ~/.pi/agent 后重跑本脚本即可自动重建链接与产物。
#   - 需要 bun 打包；--build-only 模式不需要网络。
set -euo pipefail
...
case "${1:-}" in
  ...
  "" ) ;;
  * ) echo "未知参数: $1（支持 --build-only / --skip-npm）"; exit 1 ;;
esac
...
# --- 2. npm update（含 pi-readseek 等已移出 packages 的包）---
if $SKIP_NPM; then
  echo
  echo "--- [2/4] 跳过 npm update（--skip-npm）---"
else
  echo
  echo "--- [2/4] npm update（npm/ 目录全部 dependencies）---"
  (
    cd "$NPM_DIR"
    if npm update 2>&1; then
      echo "✓ npm 依赖更新完成"
    else
      echo "⚠ npm update 返回非零，继续执行（可能个别包更新失败）"
    fi
  )
fi
...
# 校验关键产物存在且非空
for f in pi-statusline.js pi-readseek.js @narumitw-pi-btw.js; do
  if [ -s "$HERE/$f" ]; then
