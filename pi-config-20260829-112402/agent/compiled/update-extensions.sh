...
# --- asymptotic-thinking 打包（本地维护版，bun 编译）---
# 说明：自 ~/pi-agent-extensions/asymptotic-thinking 移植的渐近式思考状态机框架，
#       本地源码在 extensions/asymptotic-thinking，用项目自身 build.ts（bun build）
#       产出 dist/index.js（单文件，SYSTEM.md 已内联为 framework-rules.ts 常量，
#       不依赖运行时文件路径），复制到 compiled/ 供 settings.json 显式加载。
build_asymptotic_thinking() {
  local src_dir="$HOME/.pi/agent/extensions/asymptotic-thinking"
  [ -f "$src_dir/build.ts" ] || { echo "跳过 asymptotic-thinking（本地源码不存在）"; return; }
  echo "打包 asymptotic-thinking（本地维护版）-> asymptotic-thinking.js"
  (cd "$src_dir" \
    && bun run build \
    && cp dist/index.js "$HERE/asymptotic-thinking.js")
}
...
# 校验关键产物存在且非空
for f in pi-statusline.js pi-readseek.js billion-context-pi.js @narumitw-pi-btw.js output-compactor.js asymptotic-thinking.js; do
  if [ -s "$HERE/$f" ]; then
