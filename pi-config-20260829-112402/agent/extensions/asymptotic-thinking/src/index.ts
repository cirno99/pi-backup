...
// 扩展 SYSTEM.md（静态框架规则）——编译版内联为常量，不依赖运行时文件路径
...
${nextHint}`,
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      difficulty: StringEnum(difficultyKeys, {
        description: TOOL_DESCRIBE_DIFFICULTY,
      }),
      masterTaskType: StringEnum(masterTaskTypeKeys, {
        description: TOOL_DESCRIBE_MASTER,
      }),
      subTaskType: StringEnum(subTaskTypeKeys, {
        description: TOOL_DESCRIBE_SUB,
      }),
    }),
    // 兼容 LLM 传小写枚举：schema 校验前统一转大写
    prepareArguments(args: unknown) {
      const a = (args ?? {}) as Record<string, unknown>;
      return {
        difficulty: String(a.difficulty ?? "").toUpperCase(),
        masterTaskType: String(a.masterTaskType ?? "").toUpperCase(),
        subTaskType: String(a.subTaskType ?? "").toUpperCase(),
      };
    },
    async execute(
      _toolCallId: any,
