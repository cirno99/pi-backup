...
- **核心配置** `~/.pi/agent/` 的子集：`settings.json`、`AGENTS.md`、`pi-lsp.json`、
  ...
  `packages.lock.json`、`package-trust-audit.jsonl`，以及随运行产生的 `git/`（各扩展 git 状态缓存）、
  `pi-cache-optimizer-stats.d/`（pi-cache-optimizer 统计分片）、`cache/`、`scripts/`、
  `tool-output-artifacts/`（这些可重建，随备份走便于离线比对）
...
- **登录 provider 列表和原来不同？** 备份不含 `models-store.json` 之外的目录缓存，重新 `/login` 后会自动刷新；自定义 provider 请参考官方 docs 重新配置。
